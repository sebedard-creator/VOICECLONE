...
NOTIFICATION_ENABLE_JS = f"""
async () => {{
{NOTIFICATION_HELPER_JS}
    const helper = vcEnsureNotify();
    return await helper.enable();
}}
"""
...
def _build_ui_legacy(settings: Settings) -> gr.Blocks:
    with gr.Blocks(title="VOICECLONE-QC") as demo:
        ...
        with gr.Group():
            ...
            split_only = gr.Checkbox(
                ...
                value=False,
                info="Utilise ce mode seulement si tes sources sont deja propres: voix seule, sans musique ni bruit de fond.",
            )
        ...
        with gr.Group():
            gr.Markdown("## Nettoyage Sans Training")
            clean_only_file = gr.File(
                label="Fichier audio a nettoyer",
                file_count="single",
                type="filepath",
            )
            clean_only_button = gr.Button("Nettoyer sans slicing", variant="primary")
            clean_only_status = gr.Textbox(label="Journal nettoyage seul", lines=7, interactive=False)
            ...
            clean_only_button.click(
                run_clean_only_with_progress,
                inputs=[clean_only_file],
        ...
        with gr.Group():
            ...
            zip_upload_button.click(run_zip_upload, inputs=[drive_actor], outputs=[drive_status])
            watch_button.click(
                run_drive_watch,
            ...
            with gr.Row():
                with gr.Column():
                    gr.HTML(
                        ...
                        '<span class="vc-help" data-help="Change la hauteur de la voix en demi-tons. 0 garde la hauteur originale. Utilise +12 pour une octave plus haut, -12 pour une octave plus bas. Utile si la voix source et la voix cible ont des tessitures trÃ¨s diffÃ©rentes.">?</span>'
                        '</div>'
                    )
                ...
                with gr.Column():
                    gr.HTML(
                        ...
                        '<span class="vc-help" data-help="Dose lâ€™influence du fichier .index, donc la ressemblance avec la voix entraÃ®nÃ©e. Plus haut = plus proche du modÃ¨le, mais parfois plus dâ€™artefacts. 0.6 Ã  0.8 est souvent un bon dÃ©part.">?</span>'
                        '</div>'
                    )
                ...
                with gr.Column():
                    gr.HTML(
                        ...
                        '<span class="vc-help" data-help="ProtÃ¨ge les consonnes, respirations et sons non chantÃ©s/parlÃ©s contre les dÃ©formations. Plus haut = articulation plus stable, mais un peu moins de transformation vocale. 0.33 est un bon compromis.">?</span>'
                        '</div>'
                    )
